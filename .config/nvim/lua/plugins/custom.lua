return {
  ...
  {
    "folke/lazy.nvim", -- Dummy plugin to hold custom functions
    ...
    config = function()
      ...
      -- DemoteFromLet function
      function M.demote_from_let()
        local patterns = {
          { "let!\\?(:\\(\\w\\+\\)) { \\(.*\\) }$", "\\1 = \\2" },
          { "subject { \\(.*\\) }$", "\\1" },
          { "before { \\(.*\\) }$", "\\1" }
      ...
      vim.api.nvim_create_user_command('DemoteFromLet', function()
        M.demote_from_let()
      end, {})
      ...
      vim.keymap.set('n', 'dl', '<ESC>:DemoteFromLet<CR>', { noremap = true })
      vim.keymap.set('v', 'dl', '<ESC>:DemoteFromLet<CR>', { noremap = true })
